...
// pack(1)
type MsgPickItem struct {
	Index int
}

func (msg *MsgPickItem) Unmarshal(buf []byte) error {
	br := bytes.NewReader(buf)
...
// pack(1)
type MsgPickItemReply struct {
	Result int // -1=failed -2=money 0~237=postion
	Money  int
	Item   *item.Item
}

func (msg *MsgPickItemReply) Marshal() ([]byte, error) {
	var bw bytes.Buffer
...
func (msg *MsgDropItem) Unmarshal(buf []byte) error {
	br := bytes.NewReader(buf)
...
// pack(1)
type MsgDropItemReply struct {
	Result   int // 0=failed 1=success
	Position int
}

func (msg *MsgDropItemReply) Marshal() ([]byte, error) {
	var bw bytes.Buffer
	bw.WriteByte(byte(msg.Result))
